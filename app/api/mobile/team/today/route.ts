...
import { getTeamToday, getDefaultTeamTodayDate } from '@/lib/team/teamToday';
...
export async function GET(request: NextRequest) {
  ...
  const date =
    dateParam && /^\d{4}-\d{2}-\d{2}$/.test(dateParam.trim())
      ? dateParam.trim()
      : getDefaultTeamTodayDate();

  try {
    const result = await getTeamToday(boutiqueId, date);
    return NextResponse.json(result);
  } catch (err) {
    console.error('[mobile/team/today]', err);
